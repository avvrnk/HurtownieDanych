...
# co najmniej 100 imion i nazwisk
FIRST_NAMES = [
...
def append_pilot(outdir, pid):
    # helper that appends a single generated pilot to pilots.csv and returns its id
    path = os.path.join(outdir, "pilots.csv")
    im = rnd_name()
    ns = rnd_surname()
    lang = random.choice(["polski","angielski","włoski","hiszpański","francuski","niemiecki","rosyjski","arabski","chiński"])
...
def generate_hotels(outdir, start_id=1, min_per_city=1, max_per_city=3):
    """
    Tworzy hotele dla każdego miasta z COUNTRIES_CITIES.
    Zwraca (hotels_csv_path, hotels_info) gdzie hotels_info to lista dictów:
    { 'IdHotelu':id, 'Miasto':city, 'Kraj':country, ...}
    """
    hotels = []
    hid = start_id
    for country, cities in COUNTRIES_CITIES.items():
        for city in cities:
            # zapewnij co najmniej min_per_city hotel(i)
...
def generate_trips(n_trips, outdir, start_id=1):
    """
    Tworzy plik wycieczki z pustą kolumną IdPilota -> piloty zostaną przypisane przy generowaniu terminów
    Zwraca path i lista trip dictów (Id,Kraj,Miasto,Cena,Typ,IdPilota)
    """
    trips = []
...
def generate_terms(trips, outdir, pilots_ids, start_term_id=1, terms_per_trip=3, append_pilot_fn=None):
    """
    Dla każdego trip generuje terminy. Przypisuje pilota do tripu tak, aby jego terminy
    nie kolidowały z innymi. Jeśli brak wolnego pilota -> append_pilot_fn tworzy i dopisuje nowego pilota.
    Zwraca path do pliku terminy oraz zaktualizowaną listę trips (z IdPilota wypełnionym).
    """
    ...
    with open(path, "w", newline='', encoding="utf-8") as f:
        ...
        for trip in trips:
            ...
            for _ in range(terms_per_trip):
                ...
                # save row for writing after pilot assigned
...
def generate_kwaterowanie(trips, hotels_info, outdir):
    """
    Dla każdej wycieczki wybiera 1-3 hotele, ale tylko te które są w tym samym mieście.
    hotels_info to lista dictów wygenerowana przez generate_hotels.
    """
    # map city->hotel ids
...
def generate_rezerwacje_csv_stream(n_facts, terms_rows, trip_city_map, outpath, seed_offset=0):
    ...
    with open(outpath, "w", newline='', encoding="utf-8") as f:
        ...
        for i in range(n_facts):
            ...
            # Miejscowosc = city of corresponding trip -> need mapping from trip id to city, but terms_rows has only IdWycieczki in pos 1.
            # Caller is expected to pass terms_rows where term[1] is IdWycieczki and we will need a mapping; for simplicity here we assume caller will
            # substitute Miejscowosc by using term[2] as date and use trip-city mapping externally.
...
def main():
    ...
    # helper that appends a pilot and returns the id (closure captures next_pilot_id)
    def append_pilot_fn():
    ...
    # generate_rezerwacje_csv_stream(facts + extra, terms_rows, trip_city_map, rezerwacje_t2, seed_offset=1)

    # # 9) hotels_T2: przykładowa zmiana dimension elementu (podbijemy standard hotelu 1 i zmienimy nazwę)
